...
public class UsuarioService implements UserDetails{
        ...
        if (usuarioOpt.isPresent()) {
            ...
            try {
                ...
                if (usuario.getIntentosFallidos() >= 3) {
                    ...
                    usuarioRepository.save(usuario);
                    try {
                        emailService.sendEmail(correo, "Token Rehabilitación usuario", "Este es su token de recuperacion de usuario, ingreselo en la aplicación: " + token);
                    } catch (Exception ex) {
    ...
    public void correoRecuperacionContrasenia(String correo){
        ...
        if(usuarioOptional.isPresent()){
            ...
            usuarioRepository.save(usuario);
            try{
                emailService.sendEmail(correo,"Token Recuperación de contraseña","Este es su token de recuperacion de contraseña, ingreselo en la aplicación: " + token);
            }catch(Exception e){
                throw new IllegalArgumentException("Error al enviar correo de recuperación, consulte con el administrador");
            }
        }
